...
export async function handleCallbackQuery(container, callback) {
    const data = callback.data;
    if (!data)
        return { text: buildErrorMessage('Missing callback payload.') };
    const parts = data.split(':');
    const action = parts[0];
    const pendingTradeId = parts[1];
    if (!pendingTradeId)
        return { text: buildErrorMessage('Invalid callback payload.') };
    const existing = await container.pendingTradeStore.get(pendingTradeId);
    if (!existing)
        return { text: buildErrorMessage('This confirmation is no longer valid.') };
    if (existing.userId !== String(callback.from.id))
        return { text: buildErrorMessage('This confirmation does not belong to you.') };
    ...
    if (action === 'select_market') {
        ...
        if (!candidate)
            return { text: buildErrorMessage('Invalid market selection.') };
    ...
    if (action === 'confirm_trade') {
        ...
        if (!confirmed)
            return { text: buildErrorMessage('This confirmation has already been used or expired.') };
