...
export async function handleMessage(container, message) {
    const text = message.text?.trim();
    if (!text)
        return { text: 'Send a text command to trade.' };
    if (text === '/start')
        return { text: 'Emily trading bot is online. Send a trade like: Buy $20 YES on BTC above 100k by June' };
    if (text === '/help')
        return { text: 'Send a command like: Buy $20 YES on BTC above 100k by June' };
